...
public class Tokenizer {
    ...
    Token processCharacter(String chr, InputStreamReader file, String fileName, Token t) throws IOException, CompilerException {
        ...
        if (chr.equals("'")) {
            ...
            if (t.value.length() < 3) {
                throw new CompilerException("'" + t.value + "' is too short for char constant", fileName, line);
            }
            if (t.value.length() > 4) {
                throw new CompilerException("'" + t.value + "' is too long for char constant", fileName, line);
            }
            t = addToken(t, fileName, line);
            return t;
        }
